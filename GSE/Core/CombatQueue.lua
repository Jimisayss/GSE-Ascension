...
function CombatQueue:Initialize()
  GSE:RegisterEvent("PLAYER_REGEN_ENABLED", function()
    GSE.Log("DEBUG", "Player has left combat, processing combat queue.", "CombatQueue")
    CombatQueue:Process()
  end)
end
...
function CombatQueue:QueueAction(action)
  if not action or not action.action then
    GSE.Log("DEBUG", "Attempted to queue an invalid action.", "CombatQueue")
    return
  end
    ...
    if v then
      local success, err = pcall(function()
        if GSE.isEmpty(v.action) then
          GSE.Log("DEBUG", "Invalid OOC Queue entry", "CombatQueue")
          return
        end
      ...
      if not success then
        GSE.Log("DEBUG", "Error processing OOC Queue item: " .. tostring(err), "CombatQueue")
      end
    end
  end
end
